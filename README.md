...
- `GET /orgs/{org_id}/stream` — live annotated MJPEG feed (`multipart/x-mixed-replace`) from this org's single camera. admin/manager only (`employee` → `403`). The API opens the camera on startup and owns it for its lifetime; if no camera is available (dev/CI box, or models missing) the feature is disabled with a logged warning and this route returns `503` instead of hanging (the other routes keep working). **Auth-via-query-param:** a browser cannot set an `Authorization` header on the `<img>`/`<video>` `src` used to render MJPEG, so this route additionally accepts the token as `?token=<jwt>` (the header is preferred when both are present). Query-param tokens can leak via server access logs and referrer headers — an accepted simplification for this internal thin slice, **not** for a hardened production deployment (a hardening follow-up would use a short-lived signed stream ticket or a cookie).
